// AcrostiKit/BusinessLogic/TokenService.swift
import Foundation
import Combine
...
public class TokenService: ObservableObject {
    ...
    public func updateTokenCredentials(for token: NotionToken, newApiToken: String) {
        AcrostiKit.TokenDataController.shared.updateToken(
            id: token.id,
            name: token.name,
    ...
    public func deleteToken(_ token: NotionToken) {
        AcrostiKit.TokenDataController.shared.deleteToken(id: token.id)
        loadTokens()
        objectWillChange.send()
    }
