// AcrostiKit/DataManagement/Controllers/TokenBackup.swift
import SwiftUI
...
public final class TokenBackupManager {
    ...
    public static func exportTokens(completion: @escaping (Bool) -> Void) {
        ...
        do {
            ...
            let exportableTokens = tokens.map { token -> [String: Any] in
                return [
            ...
            // Then encode the dictionary array
            let jsonData = try JSONSerialization.data(withJSONObject: exportableTokens, options: [.prettyPrinted])
    ...
    private static func processImportedFile(at url: URL, completion: @escaping (Bool) -> Void) {
        ...
        do {
            ...
            for importToken in importedTokens {
                ...
                ), let tokenId = savedToken.value(forKey: "id") as? UUID {
