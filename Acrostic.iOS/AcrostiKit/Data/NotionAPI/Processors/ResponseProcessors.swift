...
public class DatabaseQueryProcessor: NotionPaginatedProcessor {
    ...
    public func process(results: [NotionPage]) -> Bool {
        // Convert and save pages to Core Data
        let batch = results.prefix(maxItems - processedCount)
        
        for page in batch {
            // Convert page to entity
            let _ = page.toEntity(in: context)
            
            // Create task entity if appropriate
            let task = TaskItem(
                id: page.id,
                title: page.getTitle(),
                isCompleted: page.getCompletionStatus(),
                dueDate: page.getDueDate()
            )
            
            let _ = NSManagedObject.create(
                from: task,
